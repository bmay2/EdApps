...
def get_price(soup):
	# get_price
	try:
		price_text = soup.find('meta', {'itemprop': 'price'}).get('content')
		try:
...
def get_rating(soup):		
	# get_rating
	try:
		rating_text = soup.find('div', {'itemprop':'ratingValue'}).get('title')
		rating_num = re.compile('[0-5]\.?[0-9]*').findall(rating_text)
		return float(rating_num[0])
	# except AttributeError: # this will be different in Google Play Store - fix!
	# 	try:
...
# subjects = 'Education', 'Medical', 'Books & Reference', 'Tools', 
